...
test('byteArrayToBinaryString with Uint8Array from Hex', () => {
  expect(
    byteArrayToBinaryString(new Uint8Array([0x48, 0x65, 0x6c, 0x6c, 0x6f, 0x20, 0x57, 0x6f, 0x72, 0x6c, 0x64])),
  ).toBe('0100100001100101011011000110110001101111001000000101011101101111011100100110110001100100');
});

test('byteArrayToBinaryString with Uint8Array', () => {
  expect(byteArrayToBinaryString(new Uint8Array([72, 101, 108, 108, 111, 32, 87, 111, 114, 108, 100]))).toBe(
...
test('byteArrayToBase64', () => {
  expect(byteArrayToBase64(new Uint8Array([0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09]))).toBe(
    'AAECAwQFBgcICQ==',
  );
});

test('byteArrayToBase64: Hello World', () => {
  expect(byteArrayToBase64(new Uint8Array([0x48, 0x65, 0x6c, 0x6c, 0x6f, 0x20, 0x57, 0x6f, 0x72, 0x6c, 0x64]))).toBe(
    'SGVsbG8gV29ybGQ=',
  );
});
...
test('byteArrayToString: Hello World', () => {
  expect(byteArrayToString(new Uint8Array([0x48, 0x65, 0x6c, 0x6c, 0x6f, 0x20, 0x57, 0x6f, 0x72, 0x6c, 0x64]))).toBe(
    'Hello World',
  );
});
...
test('byteArrayRange with Uint8Array from Hex from 0 , 5 elements', () => {
  expect(
    byteArrayRange(new Uint8Array([0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09]), 0, 5),
  ).toStrictEqual(new Uint8Array([0, 1, 2, 3, 4]));
});

test('byteArrayRange with Uint8Array from Hex from 1, 3 elements ', () => {
  expect(
    byteArrayRange(new Uint8Array([0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09]), 1, 3),
  ).toStrictEqual(new Uint8Array([1, 2, 3]));
});

test('byteArrayRange with Uint8Array from Hex from 2, 5 elements', () => {
  expect(
    byteArrayRange(new Uint8Array([0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09]), 2, 5),
  ).toStrictEqual(new Uint8Array([2, 3, 4, 5, 6]));
});

test('byteArrayRange with Uint8Array from Hex from 5 all remain elements', () => {
  expect(byteArrayRange(new Uint8Array([0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09]), 5)).toStrictEqual(
...
test('byteArrayToHexString', () => {
  expect(byteArrayToHexString(new Uint8Array([0x48, 0x65, 0x6c, 0x6c, 0x6f, 0x20, 0x57, 0x6f, 0x72, 0x6c, 0x64]))).toBe(
...
test('byteArrayToHexString no spaces', () => {
  expect(
    byteArrayToHexString(new Uint8Array([0x48, 0x65, 0x6c, 0x6c, 0x6f, 0x20, 0x57, 0x6f, 0x72, 0x6c, 0x64]), false),
  ).toBe('48656c6c6f20576f726c64');
});
...
test('stringSHA256 with Uint8Array from Hex', () => {
  expect(stringSHA256(new Uint8Array([0x48, 0x65, 0x6c, 0x6c, 0x6f, 0x20, 0x57, 0x6f, 0x72, 0x6c, 0x64]))).toBe(
    'a591a6d40bf420404a011733cfb7b190d62c65bf0bcda32b57b277d9ad9f146e',
  );
});

test('stringSHA256 with Uint8Array', () => {
  expect(stringSHA256(new Uint8Array([72, 101, 108, 108, 111, 32, 87, 111, 114, 108, 100]))).toBe(
    'a591a6d40bf420404a011733cfb7b190d62c65bf0bcda32b57b277d9ad9f146e',
  );
});
...
test('stringToByteArray with Hello World', () => {
  ...
  //Workaround for Jest issue with handling typed arrays
  const input = Object.values(stringToByteArray(str));
  const expected = Object.values(new Uint8Array([72, 101, 108, 108, 111, 32, 87, 111, 114, 108, 100]));
  expect(input).toEqual(expected);
});

test('stringToByteArray with ||~EVALUACIÓN~|', () => {
  const str = '||~EVALUACIÓN~|';
  //Workaround for Jest issue with handling typed arrays
  const input = Object.values(stringToByteArray(str));
  const expected = Object.values(new Uint8Array([124, 124, 126, 69, 86, 65, 76, 85, 65, 67, 73, 195, 147, 78, 126, 124]));
  expect(input).toEqual(expected);
});

//hexStringToByteArray
test('hexStringToByteArray with string', () => {
  expect(hexStringToByteArray('48656c6c6f20576f726c64')).toStrictEqual(
...
test('ASN1ECP256PublicKeyByteArrayToPEMString ', () => {
  expect(
    ASN1ECP256PublicKeyByteArrayToPEMString(
      hexStringToByteArray(
        '3059301306072a8648ce3d020106082a8648ce3d03010703420004c30866f2155c3d8b890c7a60913572088e3903a0e4f2a5f41b70072349bc8fca48e4e464b90479853442ec165dc1b0b2deb97f95fb05d090a4eec35f603a684c',
      ),
    ),
  ...
  expect(
    ASN1ECP256PublicKeyByteArrayToPEMString(
      new Uint8Array([
        ...[0x04],
        ...[
          2, 1, 144, 47, 16, 24, 147, 220, 231, 124, 38, 238, 214, 140, 205, 57, 11, 51, 200, 183, 6, 63, 124, 229, 69,
          127, 177, 153, 168, 107, 206, 231,
        ],
        ...[
          83, 135, 6, 163, 48, 165, 242, 230, 144, 180, 55, 212, 47, 154, 149, 54, 255, 81, 249, 54, 127, 199, 101, 47,
          94, 202, 33, 253, 248, 77, 205, 239,
        ],
      ]),
    ),
  ).toBe(`-----BEGIN PUBLIC KEY-----
...
test('base64ToByteArray', () => {
  expect(base64ToByteArray('AAECAwQFBgcICQ==')).toStrictEqual(
    new Uint8Array([0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09]),
  );

  expect(base64ToByteArray('SGVsbG8gV29ybGQ=')).toStrictEqual(
...
test('base64ToBase64url', () => {
  expect(base64ToBase64url('tPP6NfI3MUs5gL9O8+hK7UxWKW/c9Ju7TZ0tzOmsP9c=')).toBe(
    'tPP6NfI3MUs5gL9O8-hK7UxWKW_c9Ju7TZ0tzOmsP9c',
  );
});

//base64UrlToBase64
test('base64UrlToBase64', () => {
  expect(base64UrlToBase64('tPP6NfI3MUs5gL9O8-hK7UxWKW_c9Ju7TZ0tzOmsP9c')).toBe(
    'tPP6NfI3MUs5gL9O8+hK7UxWKW/c9Ju7TZ0tzOmsP9c=',
  );
});
...
test('COSEtoASN1PublicKey', () => {
  expect(
    COSEtoASN1PublicKey(
      credentialRegistrationData.attestationObject.authData.publicKey['-2'],
      credentialRegistrationData.attestationObject.authData.publicKey['-3'],
    ),
  ).toStrictEqual(
    new Uint8Array([
      ...[0x04],
...
test('verifyECP256Signature', () => {
  //CORRECTOS
  let publicKey = COSEtoASN1PublicKey(
    credentialRegistrationData.attestationObject.authData.publicKey['-2'],
    credentialRegistrationData.attestationObject.authData.publicKey['-3'],
  );
  expect(
    verifyECP256Signature(credentialAssertionData.response.signature, credentialAssertionData.signatureBase, publicKey),
  ).toBe(true);
  ...
  expect(
    verifyECP256Signature(credentialAssertionData.response.signature, invalidSignatureBase, invalidPublicKey),
  ).toBe(false);
  expect(verifyECP256Signature(credentialAssertionData.response.signature, invalidSignatureBase, publicKey)).toBe(
    false,
  );
  expect(
    verifyECP256Signature(
      credentialAssertionData.response.signature,
      credentialAssertionData.signatureBase,
      invalidPublicKey,
    ),
  ).toBe(false);
});
